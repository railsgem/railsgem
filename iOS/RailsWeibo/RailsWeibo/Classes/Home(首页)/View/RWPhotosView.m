...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // 初始化9个子控件
        for (int i = 0; i<9; i++) {
            RWPhotoView *photoView = [[RWPhotoView alloc] init];
...
- (void)photoTap:(UITapGestureRecognizer *)recognizer
{
    ...
    for (int i = 0; i<count; i++) {
        ...
        RWPhoto *rwphoto = self.photos[i];
        NSString *photoUrl = [rwphoto.thumbnail_pic stringByReplacingOccurrencesOfString:@"thumbnail" withString:@"bmiddle"];
...
- (void)setPhotos:(NSArray *)photos
{
    ...
    for (int i = 0; i<self.subviews.count; i++) {
        // 取出i位置对应的imageView
        RWPhotoView *photoView = self.subviews[i];
        ...
        if (i < photos.count) {
            ...
            CGFloat photoX = col * (RWPhotoW + RWPhotoMargin);
            CGFloat photoY = row * (RWPhotoH + RWPhotoMargin);
            photoView.frame = CGRectMake(photoX, photoY, RWPhotoW, RWPhotoH);
...
+ (CGSize)photosViewSizeWithPhotosCount:(int)count
{
    ...
    // 高度
    CGFloat photosH = rows * RWPhotoH + (rows - 1) * RWPhotoMargin;
    
    // 总列数
    int cols = (count >= maxColumns) ? maxColumns : count;
    // 宽度
    CGFloat photosW = cols * RWPhotoW + (cols - 1) * RWPhotoMargin;
